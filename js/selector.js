...
// Generates the controls that are inserted into the page. Returns a wrapped jQuery set that's ready to be used with appendTo and the like.
...
// Updates current_selector, the visual outline and the state of various buttons depending on the value of current_element
...
// Takes an element and walks up its hierarchy constructing a selector which would match this element (and hopefully it alone). Stops as soon as it reaches an element with a defined ID attribute or when reaching the <body>.
...
/* Sets up the button handlers:
 ...
 3. The done button sends the current selector and URL back to the extension and closes the window when a reply is received.
...
// The main function. Inserts the controls, updates the global references to them, then sets up event handlers for everything by calling setUpBodyHandlers() and setUpButtonHandlers().
function initialize() {
