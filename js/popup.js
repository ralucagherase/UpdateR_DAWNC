...
function updateButtonsState() {
    chrome.tabs.getSelected(null, function(a) {
        isPageMonitored(a.url, function(b) {
            ...
            $("#options").toggle(b);
            b || (c ? ($("#monitor_page").click(monitorCurrentPage).removeClass("inactive"),
                $("#monitor_page img").attr("src", "img/monitor.png"),
                $("#monitor_page span").text(chrome.i18n.getMessage("monitor"))) : ($("#monitor_page").unbind("click").addClass("inactive"),
    ...
    $("#notifications .notification").length ? ($("#view_all").removeClass("inactive"),
        $("#view_all img").attr("src", "img/view_all.png")) : ($("#view_all").addClass("inactive"),
        $("#view_all img").attr("src", "img/view_all_inactive.png"));
    getAllPageURLs(function(a) {
        getAllUpdatedPages(function(b) {
            a.length == b.length ? ($("#check_now").addClass("inactive"),
                $("#check_now img").attr("src", "img/refresh_inactive.png")) : ($("#check_now").removeClass("inactive"),
...
function checkAllPages() {
    getAllPageURLs(function(a) {
        ...
        0 >= a.length - b || ($("#check_now").unbind("click"),
            a = 0 < b ? {
                height: "2.7em",
                opacity: .01
            } : {
