...
/* Splits "replace" runs so that they align at the first tag. This is necessary for the recursive differ to be able to match respective elements. Non-replace
 runs are left as is. Example run before replacement:
...
// Recursively diffs "replace" runs. The contents of elements with matching tags are passed to calculateHtmlDiff(), the result of the diff is reinserted into
// src[_hashed] and dst[_hashed] and the run is converted to "equal". Elements with mismatched tags are converted into "delete"/"insert" runs. Non-replace
...
function recurseHtmlDiff(opcodes, src, dst, src_hashed, dst_hashed, loose) {
    ...
    eachOpcode(opcodes, function(opcode, src_start, src_end, dst_start, dst_end) {
        if (opcode[0] != 'replace') {
            ...
            // It is often the case that minor changes in the first/last item of a run produce an overly greedy replace subsequence. Here we chop off the
            // beginning or end of either the deleted or the inserted array to make sure both are of the same size and hopefully aligned before recursing.
...
// Clusters interleaved "delete" and "insert" runs into contiguous sequences of "delete" or "insert" runs. For example, given the following sequence:
...
// Assembles a diff opcode sequence into a string list whose concatenation is a valid HTML string, by surrounding sequences of deletion or insertion runs with <del> or <ins>, respectively.
...
// Walks the assembled HTML diff list and wraps the contents of all <td>s inside an insert or delete sequence with insert or delete tags, respectively.
...
// Calculates the diff between two HTML strings, src and dst, and returns a compiled version with <del> and <ins> tags added in the appropriate places.
// Returns null if there's an error in the diff library. Called recursively to diff each subtree. Uses difflib for calculating the diff. If loose_compare is true, comparisons use cleanHtmlPage(). See the documentation for that function for details.
...
// Calculates the diff between two text strings, src and dst, and returns a compiled version with <del> and <ins> tags added in the appropriate places.
// Returns null if there's an error in the diff library. The returned string is valid HTML, with <, > and & escaped, as well as newlines converted to <br />.
...
// Generates the control block that is added to the diffed page. Includes a link to the original page, as well as a toggler to show/hide deletions. The returned value is a jQuery-wrapped div with the appropriate event(s) already attached.
...
// Searches src and dst for a <base> tag, and returns the URL pointed to by the first one found. If none found, returns the passed URL.
...
// Returns a concatenation of the content of all <style> tags in the HTML, separated by newlines.
...
// Returns a jQuery-wrapped list of <link> elements that point to stylesheets in the supplied HTML string.
...
// Takes a URL, the source and destination HTML strings, and a MIME type. Diffs the strings either as text or as HTML depending on the type, inserts the result into the current page with appropriate changes and UI controls, then
// scrolls to the first change. If loose is true and the diff is in HTML mode, cleanHtmlPage() is used to ignore all but alphabetic changes.
...
// Retrieves a saved snapshot of the URL, then the current live version, and runs a diff between them using applyDiff(). The content-type header of the
// live version is used to select whether to run a text or an html diff. If no saved snapshot is available, displays a diff_coruption error message in the first div of the page.
